...
def updateCb(client, callback_query,redis):
  ...
  if re.search("^ring.pyplay$",date):
    ...
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("سهل 1️⃣", callback_data="ring=1="+str(userID))
    ],[
        InlineKeyboardButton("متوسط 2️⃣", callback_data="ring=2="+str(userID))
    ],[
        InlineKeyboardButton("صعب 3️⃣", callback_data="ring=3="+str(userID))
    ],[
        InlineKeyboardButton("صعب جداً 4️⃣", callback_data="ring=4="+str(userID))
    ]])
  ...
  if re.search("ring=",date):
    ...
    if userID == int(p1):
        ...
        while i < df:
            ...
            ar.append(a)
            a = []
        ar.append([InlineKeyboardButton("📣",url="t.me/killer_source")])
  ...
  if re.search("rg=",date):
    ...
    if int(wonNum) == int(k):
      ...
      txt += f"حظ اوفر ⛔️\n\n{tx}\n⎯ ⎯ ⎯ ⎯"
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("اللعب مجدداً",callback_data="ring.pyplay")]])
...
🔴꒐ تعادل"""
  ...
  if re.search("st1=",date):
    ...
    kb = InlineKeyboardMarkup([
      ...
      [InlineKeyboardButton("📣",url="t.me/killer_source")]
      ])
